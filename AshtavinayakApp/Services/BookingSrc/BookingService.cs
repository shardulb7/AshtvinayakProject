...
{
    public partial record BookingService : IBookingService
    {
        private readonly AshtvinayakTravelContext _context;
        public BookingService(AshtvinayakTravelContext ashtvinayakTravelContext)
        {
            _context = ashtvinayakTravelContext;
        }

        public async Task<(bool Success, string Message, object Data)> CreateBookingWithSeatsAsync(BookingRequestDto request)
        {
            ...
            try
            {
                ...
                trip.AvailableSeats -= request.SeatNumbers.Count;
                ...
                var isSent = await SendSmsAsync(user.PhoneNumber, message);
            ...
            catch (Exception ex)
            {
                ...
                string innerError = ex.InnerException?.Message ?? "No inner exception";
                return (false, $"Internal server error: {ex.Message}", new { InnerError = innerError });
            }
        }
        ...
        public async Task<(bool Success, string Message, object Data)> BookCarAsync(CarBookingDTOModel request)
        {
            ...
            catch (Exception ex)
            {
                ...
                return (false, "Internal server error", new { Error = innerError });
            }
        }
    ...
    .ToDictionaryAsync(
        ...
        g => g.Select(t => new Transaction
        {
            ...
            {
                return (false, "Internal server error.", new { Error = ex.Message });
            }
        }
        ...
        public async Task<(bool Success, string Message, object? Data)> UpdatePaymentAsync(Transaction dto)
        {
            ...
            {
                return (false, "Error updating payment.", new { Error = ex.Message });
            }
        }
        /// <summary>
        /// Get Invoice
        ...
        public async Task<(bool Success, string Message, object Data)> GetInvoiceAsync(int bookingId)
        {
            try
            {
                ...
                //Customer Info
                builder.Replace("%Customer Name%", booking.User.UserName.ToString());
                builder.Replace("%Consignee Name%", booking.User.UserName.ToString());
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    string url = $"{baseUrl}?user=iTasT&pwd=Akshay@7995&senderid=ITAST&CountryCode=91" +
                                 ...
                                 $"&smstype = 9 & pe_id = 1701174522321104846 & template_id = 1707176034138969758";
